<template>
  <b-container>
    ...
    <table class="table table-striped table-hover">
      ...
      <tbody>
        <tr v-for="(question, idx) in quiz.questions" :key="idx" :class="[questionRowClass(question)]">
          <td>{{question.item}}</td>
          <td>
            <select class="form-control score-select" v-model="question.score" :disabled="question.result !== undefined">
    ...
    <div class="form-group" v-if="!quiz.finalScore">
      ...
      <input id="highscoreName" type="text" class="form-control mb-3" placeholder="Name" :value="name">
...
.score-select {
  // or numbers won't be fully visible (might need a better fix for really small screens)
  min-width: 65px;
}
</style>
